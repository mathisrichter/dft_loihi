...
class Plotter():
    ...
    def add_node_plot(self, node):
        self.add_field_plot(node)

    def add_field_plot(self, field):
        """Creates plots for all probes of the node."""

        if field.probe_spikes is None:
            print("Error: Field or node does not contain probes.")

        fig = plt.figure(figsize=(18,10))
        fig.suptitle(field.name)

        ax0 = plt.subplot(3, 1, 1)
        field.probe_current.plot()
        plt.xlabel('Time steps')
        plt.ylabel('Current')
        plt.title('Current')

        ax1 = plt.subplot(3, 1, 2)
        field.probe_voltage.plot()
        plt.xlabel('Time steps')
        plt.ylabel('Voltage')
        plt.title('Voltage')

        ax2 = plt.subplot(3, 1, 3)
        field.probe_spikes.plot()
        plt.xlabel('Time steps')
        plt.ylabel('Neuron index')
        plt.title('Spikes')
        plt.ylim(0, field.number_of_neurons)
